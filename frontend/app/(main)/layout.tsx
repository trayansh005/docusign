...
import {
	Inter, JetBrains_Mono, Dancing_Script, Great_Vibes, Allura,
	Alex_Brush, Amatic_SC, Caveat, Kaushan_Script, Pacifico, Satisfy, Permanent_Marker,
...
const dancingScript = Dancing_Script({ variable: "--font-dancing-script", subsets: ["latin"], display: "swap" });
...
const alexBrush = Alex_Brush({ variable: "--font-alex-brush", subsets: ["latin"], weight: ["400"], display: "swap" });
const amaticSC = Amatic_SC({ variable: "--font-amatic-sc", subsets: ["latin"], weight: ["400", "700"], display: "swap" });
const caveat = Caveat({ variable: "--font-caveat", subsets: ["latin"], weight: ["400", "700"], display: "swap" });
const kaushanScript = Kaushan_Script({ variable: "--font-kaushan-script", subsets: ["latin"], weight: ["400"], display: "swap" });
const pacifico = Pacifico({ variable: "--font-pacifico", subsets: ["latin"], weight: ["400"], display: "swap" });
const satisfy = Satisfy({ variable: "--font-satisfy", subsets: ["latin"], weight: ["400"], display: "swap" });
const permanentMarker = Permanent_Marker({ variable: "--font-permanent-marker", subsets: ["latin"], weight: ["400"], display: "swap" });
...
const fontVars = [
	inter, jetbrainsMono, dancingScript, greatVibes, allura,
	alexBrush, amaticSC, caveat, kaushanScript, pacifico, satisfy, permanentMarker,
].map((f) => f.variable).join(" ");

export default async function MainLayout({ children }: { children: React.ReactNode }) {
	return (
		<html lang="en" className="dark">
			<body className={`${fontVars} min-h-screen flex flex-col scrollbar-thin`}>
