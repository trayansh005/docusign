...
export const RecipientManager: React.FC<RecipientManagerProps> = ({
	...
	return (
		<div className={`bg-slate-900 border border-slate-700 rounded-lg p-6 ${className}`}>
			...
			{sortedRecipients.length > 0 && (
				<div className="space-y-3 mb-6">
					{sortedRecipients.map((recipient, index) => (
						<div
							key={recipient.id}
							className="flex items-center gap-4 p-4 border border-slate-700 rounded-lg bg-slate-800"
						>
							{/* Order controls */}
							<div className="flex flex-col gap-1">
								<button
									...
									className="p-1 text-gray-500 hover:text-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
									title="Move up"
								>
							...
							{/* Order number */}
							<div className="flex-shrink-0 w-8 h-8 bg-blue-600 border border-blue-500 rounded-full flex items-center justify-center text-sm font-medium text-white">
							...
							<div className="flex-1 min-w-0">
								<div className="flex items-center gap-2">
									...
									<span className="font-medium text-white truncate">{recipient.name}</span>
								</div>
								<div className="flex items-center gap-2 mt-1">
									<Mail className="w-4 h-4 text-gray-500" />
									<span className="text-sm text-gray-400 truncate">{recipient.email}</span>
								</div>
							</div>
			...
			{/* Add recipient form */}
			{isAdding ? (
				<div className="border border-slate-700 rounded-lg p-4 mb-4 bg-slate-800">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div>
							...
							<input
								...
								className="w-full px-3 py-2 border border-slate-600 rounded-md bg-slate-700 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
								disabled={disabled}
							/>
						</div>
						<div>
					...
					<div className="flex gap-2 mt-4">
						...
						<button
							...
							disabled={disabled}
							className="px-4 py-2 bg-slate-700 text-gray-200 rounded-md hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium"
						>
							Cancel
						</button>
					</div>
				...
				<button
					...
					disabled={disabled}
					className="w-full flex items-center justify-center gap-2 p-4 border-2 border-dashed border-slate-600 rounded-lg text-gray-400 hover:border-slate-500 hover:text-gray-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
				>
			...
			<div className="mt-4 p-3 bg-blue-900/30 border border-blue-800 rounded-lg">
				<p className="text-sm text-blue-200">
					<strong>Signing Order:</strong> Recipients will sign in the order shown above. Use the
					arrow buttons to reorder recipients. Each person must wait for the previous person to
					sign.
				</p>
			</div>
		</div>
