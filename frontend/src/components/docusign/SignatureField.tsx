...
export const SignatureField: React.FC<SignatureFieldProps> = ({
	...
	const fieldConfig = {
		...
		phone: {
			label: user?.phoneNumber || "(555) 123-4567",
			placeholder: "PHONE",
			fontFamily: "",
			minWidth: 6,
			minHeight: 2,
	...
	const fontSize = useMemo(() => {
		...
		const calculatedSize = Math.min(Math.max(pixelHeight * baseFactor, minSize), maxSize);
		...
		(e: MouseEvent) => {
			...
			requestAnimationFrame(() => {
				...
				if (dragState.current.isDragging) {
					...
					const newX = Math.max(
						0,
						Math.min(100 - field.wPct, dragState.current.startFieldX + deltaXPct)
					);
					const newY = Math.max(
						0,
						Math.min(100 - field.hPct, dragState.current.startFieldY + deltaYPct)
					);
					...
					const newW = Math.max(
						config.minWidth,
						Math.min(100 - field.xPct, dragState.current.startFieldW + deltaXPct)
					);
					const newH = Math.max(
						config.minHeight,
						Math.min(100 - field.yPct, dragState.current.startFieldH + deltaYPct)
					);
	...
	const textStyle: React.CSSProperties = {
		...
		textShadow:
			field.type === "signature" || field.type === "initial" ? "0 1px 2px rgba(0,0,0,0.1)" : "none",
	};
	...
	return (
		<div
			...
			title={
				editable ? `${field.type.toUpperCase()} field - Click to select, drag to move` : displayText
			}
		>
			{/* Field Content */}
