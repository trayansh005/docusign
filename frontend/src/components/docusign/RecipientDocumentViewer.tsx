...
export interface RecipientDocumentViewerRef {
  ...
  getPendingSignatures: () => Record<string, { value: string; fontId?: string; text?: string; imageData?: string }>;
}
  ...
  ({ template, onFieldUpdate }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      ...
      getPendingSignatures: () => {
        console.log('[RecipientDocumentViewer] getPendingSignatures called, count:', Object.keys(pendingSignaturesRef.current).length);
    ...
    const renderField = (field: SignatureField) => {
      ...
      const hasSigned = (displayValue && displayValue.trim() !== "") ||
        ...
        >
          <div className="flex flex-col items-center justify-center p-2 text-center min-w-0 w-full h-full">
            {(() => {
              ...
              if (isImage && !isTypedSignature && (field.type === "signature" || field.type === "initial")) {
                return (
                  <img
                    src={value}
