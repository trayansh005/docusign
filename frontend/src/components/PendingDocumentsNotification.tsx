    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                title={unreadCount > 0
                    ? `You have ${unreadCount} new document${unreadCount > 1 ? "s" : ""} waiting for your signature`
                    : `${pendingCount} pending document${pendingCount !== 1 ? "s" : ""}`
                }
            >
                {/* Bell Icon with animation */}
                <svg
                    className={`w-6 h-6 transition-all duration-300 ${unreadCount > 0 ? "text-yellow-400 animate-pulse" : "text-gray-400"}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                                        ...
                                        <div className="flex items-start justify-between gap-3">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mt-2">
                                                    <span
                                                        className={`text-xs px-2.5 py-1 rounded-full font-medium ${item.status === "final"
                                                    ...
                                                    {item.myRecipientInfo?.signatureStatus && (
                                                        <span
                                                            className={`text-xs px-2.5 py-1 rounded-full font-medium ${item.myRecipientInfo.signatureStatus === "signed"
                                                                ? "bg-green-500/20 text-green-400 border border-green-500/30"
                                                                : "bg-orange-500/20 text-orange-400 border border-orange-500/30"
                                                                }`}
                                                        >
                                                            {item.myRecipientInfo.signatureStatus === "signed" ? "✓ Signed" : "⏳ Pending"}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
