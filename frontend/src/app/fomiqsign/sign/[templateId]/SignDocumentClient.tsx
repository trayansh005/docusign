...
import { RecipientDocumentViewerRef } from "@/components/docusign/RecipientDocumentViewer";
...
export default function SignDocumentClient() {
  ...
  const recipientViewerRef = useRef<RecipientDocumentViewerRef>(null);
  ...
  const handleSaveSignatures = async () => {
    ...
    // Check if it's the user's turn to sign
    if (!canSign) {
      alert(
        signingMessage ||
    ...
    setIsSaving(true);
    try {
      // Get pending signatures (contains imageData for typed signatures)
      const pendingSignatures = recipientViewerRef.current?.getPendingSignatures() || {};

      // Build signature payload - use imageData from pending signatures if available
      const signatures = myFields.map((field: SignatureField) => {
        const pending = pendingSignatures[field.id];
        // Use imageData if available (for typed signatures), otherwise use field.value (for drawn signatures)
        ...
        return {
          ...
          signatureImageBuffer: finalValue,
      ...
      console.log("Signatures being sent to backend:", {
        count: signatures.length,
        firstSignature: signatures[0]
          ? {
            fieldId: signatures[0].fieldId,
            type: signatures[0].type,
            hasImage: signatures[0].signatureImageBuffer.startsWith("data:image"),
            fontId: signatures[0].fontId,
          }
          : "No signatures",
      });
