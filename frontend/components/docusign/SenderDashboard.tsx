...
export const SenderDashboard: React.FC = () => {
		...
		try {
			...
			const documentsWithProgress = response.data.map((doc) => {
				if (doc.recipients && doc.recipients.length > 0) {
					...
					const signedRecipients = doc.recipients.filter(
						(r) => r.signatureStatus === "signed"
					).length;
					const completionPercentage =
						totalRecipients > 0 ? Math.round((signedRecipients / totalRecipients) * 100) : 0;

					// Find next recipient to sign
					const sortedRecipients = [...doc.recipients].sort(
	...
	const filteredDocuments = documents.filter((doc) => {
		...
		if (filter === "pending")
			return !doc.progress?.isComplete && doc.recipients && doc.recipients.length > 0;
		return true;
	});
	...
	const stats = {
		...
		pending: documents.filter(
			(d) => !d.progress?.isComplete && d.recipients && d.recipients.length > 0
		).length,
			...
			<div className="max-w-7xl mx-auto">
				<div className="mb-8">
					...
					<p className="text-gray-400">
						Track the progress of your sent documents and see who needs to sign next
					</p>
				</div>
								...
								<div className="flex items-start justify-between mb-4">
									<div className="flex items-center flex-1 min-w-0">
										...
										<div className="ml-3 flex-1 min-w-0">
											...
											<p className="text-sm text-gray-400">{getStatusText(doc)}</p>
										</div>
									</div>
												...
												<div className="space-y-1">
													{doc.recipients.slice(0, 3).map((recipient) => (
														<div key={recipient.id} className="flex items-center text-xs">
															...
															<span className="text-white truncate flex-1">{recipient.name}</span>
							...
							<div className="p-6">
								...
								{selectedDocument.recipients && selectedDocument.recipients.length > 0 ? (
									...
									<div className="text-center py-8">
										...
										<p className="text-gray-400">
											This document hasn&apos;t been sent to anyone yet.
										</p>
									</div>
								)}
